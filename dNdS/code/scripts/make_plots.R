...
omega <- read.table(snakemake@input[[1]], sep = ",", col.names = c("omega", "OG"))
...
df <- filter(final, final$omega < 100)
...
p <- ggplot(df, aes( x = Classification, y = omega, fill = Classification)) +
  geom_violin()+
  geom_sina()+	
  labs(title="dN/dS in Pantranscriptome", x = "Classification", y = "dN/dS")+
